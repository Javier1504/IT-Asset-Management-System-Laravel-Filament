...
@section('title', 'Detail Office Asset Stock Opname')
...
<div class="container-fluid py-4">
    ...
    @php
        ...
        $items = isset($items) ? collect($items) : collect([$item]);
        ...
        $assetNameHeader = $item->snapshot_asset_name
            ?? $item->aset?->jenisAset?->name_jenis
            ?? $item->officeAset?->aset?->jenisAset?->name_jenis
            ?? 'Office Asset';

        $assetNumberHeader = $item->snapshot_asset_number ?? $item->aset?->nomor_aset ?? '-';
        $locationName = $locationName
            ?? $item->snapshot_location_name
            ?? $item->officeAset?->lokasi?->lokasi
            ?? 'Tanpa Lokasi';
        $locationKey = (string) ($locationKey ?? ($item->location_id ?: 'tanpa-lokasi'));
        ...
        $focusItemId = request('focus_item_id', $item->id ?? null);
        $backUrl = route('stock-opnames.office-location.show', [$stockOpname->id, $locationKey]);
    @endphp

    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            <h4 class="mb-1">{{ $assetNameHeader }}</h4>
            <p class="text-muted mb-0">
                Lokasi: {{ $locationName }}
                |
                Nomor Aset: {{ $assetNumberHeader }}
                |
        ...
        <div class="d-flex gap-2 align-items-center flex-wrap">
            <span class="badge bg-primary">Office Asset</span>
            <a href="{{ $backUrl }}" class="btn btn-outline-secondary">
                Kembali ke Daftar Office Asset
            </a>
        </div>
    </div>
    ...
    <div class="alert alert-info border-0 shadow-sm">
        <strong>Checklist Office Asset.</strong>
        menampilkan detail Office Asset beserta checklist komponennya.
    </div>
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('stock-opnames.office-items.show', [$stockOpname->id, $item->id]) }}" class="row g-2">
                <div class="col-md-5">
                    <input type="text"
                           name="search"
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('stock-opnames.office-items.show', [$stockOpname->id, $item->id]) }}" class="btn btn-outline-secondary">
                        Reset
                    </a>
                </div>
            </form>
    ...
    @forelse($items as $item)
        ...
        <div class="card border-0 shadow-sm mb-3" id="stock-opname-item-{{ $item->id }}">
            <div class="card-body">
                ...
                <form action="{{ route('stock-opnames.items.update', [$stockOpname->id, $item->id]) }}"
                      method="POST"
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-3">
                            <label class="form-label">Lokasi Sesuai</label>
                            ...
                            <small class="text-muted">Apakah aset berada di lokasi yang benar.</small>
                        </div>
    ...
    @empty
        <div class="alert alert-info">
            Office Asset tidak ditemukan pada lokasi ini.
        </div>
    @endforelse
</div>
