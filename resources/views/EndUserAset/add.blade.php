        ...
        <div class="container-fluid p-0">
            <div class="w-100 p-2 bg-white">
                <div class="w-100 bg-body-tertiary rounded-4 p-2 py-md-3 py-xl-4 pb-5">
                    <form method="POST" action="{{ route('end-user-aset.store') }}" id="advancedForm"
                                    ...
                                    <div class="row row-cols-1 row-cols-md-2 g-3">
                                        <div class="col-md-12">
                                            <div class="d-flex flex-column gap-1 mb-2">
                                                <h1 class="fs-5 fw-medium mb-0">Data End-User Aset</h1>
                                                <p class="fs-6 fw-medium text-secondary mb-0">
                                                    Pilih dan masukkan data aset End-User.
                                                </p>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-select" id="klasifikasi_laptop_id"
                                                ...
                                                <option value="" selected>Pilih Klasifikasi Untuk Laptop</option>
                                                @foreach ($klasifikasiLaptops as $klasifikasi)
                                                    <option value="{{ $klasifikasi->id }}"
                                                        {{ old('klasifikasi_laptop_id') == $klasifikasi->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" name="serial_number" id="serial_number"
                                                value="{{ old('serial_number') }}" class="form-control"
                                        ...
                                        <div id="status_retirement_wrapper" class="mb-4" style="display: none;">
                                            <label for="status_retirement" class="form-label">Status Retirement</label>
                                            <select name="status_retirement" id="status_retirement" class="form-select">
                                                <option value="">-- Pilih Status Retirement --</option>
                                                @foreach ($statusesRetirement as $sr)
                                                    <option value="{{ $sr }}"
                                                        {{ old('status_retirement') === $sr ? 'selected' : '' }}>
                                                        {{ ucfirst($sr) }}</option>
                                                @endforeach
                                            </select>
...
<script>
    ...
    function toggleRetirement() {
        ...
        if (!wrap) return;
        var current = getCheckedStatusAset();
        var show = (current === 'retirement');
        wrap.style.display = show ? '' : 'none';
        if (!show) {
            var sr = document.getElementById('status_retirement');
            if (sr) sr.value = '';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        radios.forEach(function(r) {
            r.addEventListener('change', toggleRetirement);
        });
        toggleRetirement(); // init
    });
</script>
@endsection
