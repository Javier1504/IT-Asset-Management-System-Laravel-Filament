...
@section('title', 'Buat BAST Persetujuan Aset Pribadi')
    ...
    <header class="qn-header z-1 sticky-top p-md-3 py-3 px-xl-5 bg-white">
        <div class="container-fluid d-grid d-flex justify-content-between align-items-center position-relative">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="m-0 fs-6 fw-medium">Buat BAST Persetujuan Aset Pribadi</span>
            </div>
        ...
        <div class="container-fluid p-0">
            <div class="w-100 p-2 bg-white">
                <div class="w-100 bg-body-tertiary rounded-4 p-2 py-md-3 py-xl-4 pb-5">
                    <form method="POST" action="{{ route('bast-persetujuan-asetpribadi.store') }}" id="bastForm"
                                    ...
                                    <div class="row row-cols-1 row-cols-md-2 g-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <small class="text-muted">
                                                <i class="sym sym-info-circle"></i>
                                                Pilih penerima terlebih dahulu sebelum menambah request
                                            </small>
                                        </div>
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-12 d-flex justify-content-end gap-2">
                                            <button type="button" class="btn btn-primary" id="btnTambahRequest">
                                                        ...
                                                        <div class="table-responsive mt-4"
                                                            ...
                                                            <table class="table table-bordered align-middle">
                                                                ...
                                                                <tbody id="asetpribadiRequestTable">
                                                                    @forelse ($asetpribadiRequests as $request)
                                                                        <tr>
                                                                            <td>
                                                                                <button type="button"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function populateUserData(selectId, jobRoleId, alamatId) {
                ...
                select.addEventListener('change', function() {
                    ...
                    jobRoleInput.value = opt?.getAttribute('data-job-role') || '';
                    alamatInput.value = opt?.getAttribute('data-alamat') || '';
                });
            }
            ...
            document.getElementById('btnTambahRequest').addEventListener('click', function() {
                const pihakKedua = document.getElementById('inputUser2').value;

                if (!pihakKedua) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Pilih Pihak Kedua',
                        text: 'Pilih Pihak Kedua sebelum menambah request.'
                    });
                    return;
                }

                selectedPihakKeduaId = pihakKedua;
                loadRequests();
                new bootstrap.Modal(document.getElementById('asetpribadiModal')).show();
            });


            document.getElementById('inputUser2').addEventListener('change', function() {
                if (selectedRequests.length === 0) {
                    selectedPihakKeduaId = this.value;
                    return;
                }

                Swal.fire({
                    title: 'Ubah Pihak Kedua?',
                    text: 'Semua request yang dipilih akan dihapus.',
                    icon: 'warning',
                ...
                fetch(`${url}?user_id=${selectedPihakKeduaId}&search=${encodeURIComponent(search)}`)
                    ...
                    .then(res => {
                        ...
                        res.data.forEach(request => {
                            ...
                            if (Array.isArray(request.aset_pribadi)) {
                                asetHtml = request.aset_pribadi.map((a, i) => `
                            <div class="mb-1">
                                <strong>${a.nama || '-'}</strong><br>
                                <small>S/N: ${a.no_seri || '-'}</small><br>
                                <small>MAC: ${a.mac_address || '-'}</small>
                            </div>
                            ${i < request.aset_pribadi.length - 1 ? '<hr class="my-1">' : ''}
                        `).join('');
                            }
                        ...
                        <tr>
                            <td>
                                <button type="button"
                                    class="btn btn-primary btn-sm pilihRequest"
                                    data-id="${request.id}"
                                    data-user="${request.user?.name_karyawan || '-'}"
                                    data-jabatan="${request.jabatan_user || '-'}"
                                    data-divisi="${request.divisi || '-'}"
                                    data-tanggal="${new Date(request.created_at).toLocaleDateString('id-ID')}"
                                    data-aset='${JSON.stringify(request.aset_pribadi || [])}'>
                                    Pilih
                                </button>
                            </td>
            ...
            function updateSelectedRequestsTable() {
                const tbody = document.getElementById('selectedRequestsBody');
                tbody.innerHTML = '';

                selectedRequests.forEach((r, i) => {
                    let asetHtml = r.aset.map((a, idx) => `
                <div>
                    <strong>${a.nama || '-'}</strong><br>
                    <small>S/N: ${a.no_seri || '-'}</small><br>
                    <small>MAC: ${a.mac_address || '-'}</small>
                </div>
                ${idx < r.aset.length - 1 ? '<hr class="my-1">' : ''}
            `).join('');

                    tbody.innerHTML += `
                <tr>
                    <td>
                        <button class="btn btn-danger btn-sm" onclick="removeRequest(${i})">
