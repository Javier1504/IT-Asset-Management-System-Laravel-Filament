...
<title>Pratinjau Surat Berita Acara Serah Terima Aset</title>
        ...
        <div class="container">
            <div class="row row-cols-1 gy-3 p-3 p-lg-4">
                    ...
                    <div class="d-flex flex-column gap-2">
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                            <ol class="breadcrumb mb-1">
                                @if (request()->routeIs('bast-aset.show', $bast->id))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Berita
                                            Acara</a></li>
                                @elseif(request()->routeIs('daftar-tanda-tangan.detailBast', $bast->id))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Tanda
                                            Tangan</a></li>
                                @endif
                                @if (request()->routeIs('bast-aset.show', $bast->id))
                                    <li class="breadcrumb-item"><a href="{{ route('bast-aset.index', $bast->id) }}">BA Serah
                                            Terima</a></li>
                                @elseif(request()->routeIs('daftar-tanda-tangan.detailBast'))
                                    <li class="breadcrumb-item"><a
                                            href="{{ route('daftar-tanda-tangan.bast', $bast->id) }}">BA Serah Terima</a>
                                    </li>
                                @endif
                ...
                <div class="d-flex align-items-center justify-content-between gap-2 px-0">
                    <h4 class="m-0">Surat Berita Acara Serah Terima Aset</h4>

                    @php
                        $showButton = false;
                        if (auth()->id() === $bast->user_pihak_pertama_id && !$bast->tanda_tangan_pihak_pertama) {
                            $showButton = true;
                        }
                        if (auth()->id() === $bast->user_pihak_kedua_id && !$bast->tanda_tangan_pihak_kedua) {
                            $showButton = true;
                        }
                    @endphp
                    ...
                    @if ($showButton)
                        <form
                            action="{{ request()->routeIs('bast-aset.show', $bast->id) || request()->routeIs('bast-aset.edit', $bast->id) || request()->routeIs('bast-aset.destroy', $bast->id) || request()->routeIs('bast-aset.create') ? route('bast-aset.sign', $bast->id) : (request()->routeIs('daftar-tanda-tangan.detailBast', $bast->id) ? route('daftar-tanda-tangan.signBast', $bast->id) : '') }}"
                ...
                <div class="card shadow-sm border-0 rounded-4 p-3">
                    <div class="card-body">
                        ...
                        <div
                            ...
                            <iframe id="printFrame" src="{{ route('bast-aset.download', $bast->id) }}"
