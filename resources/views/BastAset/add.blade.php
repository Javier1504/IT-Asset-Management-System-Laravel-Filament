...
@section('content')
    ...
    <header class="qn-header z-1 sticky-top p-md-3 py-3 px-xl-5 bg-white">
        <div class="container-fluid d-grid d-flex justify-content-between align-items-center position-relative">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="m-0 fs-6 fw-medium">Buat Surat BAST Aset</span>
            </div>
        ...
        <div class="container-fluid p-0">
            <div class="w-100 p-2 bg-white">
                <div class="w-100 bg-body-tertiary rounded-4 p-2 py-md-3 py-xl-4 pb-5">
                    <form method="POST" action="{{ route('bast-aset.store') }}" id="advancedForm"
                                ...
                                <div class="card-body p-3 px-md-4 p-xl-4 px-xl-5">
                                    <div class="row row-cols-1 row-cols-md-2 g-3">
                                            ...
                                            <select class="form-select" id="inputUser1" name="user_pihak_pertama_id">
                                                ...
                                                @foreach ($users->where('role', 'admin') as $user)
                                                    <option value="{{ $user->id }}"
                                                        ...
                                                        {{ old('user_pihak_pertama_id') == $user->id ? 'selected' : '' }}>
                                                        {{ $user->name_karyawan }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-select" id="inputUser2" name="user_pihak_kedua_id">
                                                ...
                                                @foreach ($penerima as $user)
                                                    <option value="{{ $user->id }}"
                                                        data-job-role="{{ $user->job_role }}"
                                                        data-alamat="{{ $user->alamat }}"
                                                        {{ old('user_pihak_kedua_id') == $user->id ? 'selected' : '' }}>
                                                        {{ $user->name_karyawan }}
                                                    </option>
                                                @endforeach
                                            </select>
                                                        ...
                                                        <div class="tab-content mt-3" id="assetTabContent">
                                                            ...
                                                            <div class="tab-pane fade show active" id="endUserAset"
                                                                ...
                                                                <div class="table-responsive mt-4" id="endUserAsetTable"
                                                                    style="max-height:350px; overflow-y:auto; display:block;">
                                                                    <!-- Table responsive untuk scroll -->
                                        ...
                                        <div class="col-md-12 mt-3">
                                            <div class="table-responsive mt-2" id="endUserAsetTable" style="max-height:500px; overflow-y:auto; display:block;">                                                <table class="table table-bordered align-middle">
                                                    <thead class="align-middle">
                                                        <tr class="table-light">
                                                        ...
                                                        @if (in_array($status, ['terpakai']))
                                                            <div class="form-check">
                                                                <input class="form-check-input" type="radio"
                                                                    ...
                                                                    value="{{ $status }}"
                                                                    {{ old('status', 'terpakai') == $status ? 'checked' : '' }}
                                                                    required>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data && response.data.length > 0) {
                            ...
                            tbody =
                                '<tr><td colspan="9" class="text-center">Tidak ada data aset yang available</td></tr>';
                        }
                    ...
                    error: function(xhr, status, error) {
                        ...
                        alert('Gagal mencari data aset: ' + error);
                    }
                });
            });
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                $.each(response.data, function(index, aset) {
                    ...
                    switch (aset.status_aset) {
                        case 'stock':
                            statusBadge = `<span class="badge text-primary bg-primary bg-opacity-10 border border-primary">Stock</span>`;
                            break;
                        case 'diperbaiki':
                            statusBadge = `<span class="badge text-warning bg-warning bg-opacity-10 border border-warning">Diperbaiki</span>`;
                            break;
                        case 'terpakai':
                            statusBadge = `<span class="badge text-success bg-success bg-opacity-10 border border-success">Terpakai</span>`;
                            break;
                        case 'dipinjam':
                            statusBadge = `<span class="badge text-info bg-info bg-opacity-10 border border-info">Dipinjam</span>`;
                            break;
                        case 'retirement':
                            statusBadge = `<span class="badge text-danger bg-danger bg-opacity-10 border border-danger">Retirement</span>`;
                            break;
                        case 'dihibahkan':
                            statusBadge = `<span class="badge text-success bg-success bg-opacity-10 border border-success">Dihibahkan</span>`;
                            break;
                        default:
                ...
                tbody = '<tr><td colspan="9" class="text-center">Tidak ada data aset yang available</td></tr>';
            }

            $('#asetTableBody').html(tbody);
            attachSelectAssetEvents();

            // Sembunyikan baris yang sudah dipilih setelah data selesai dimuat
            const selectedAssets = JSON.parse(localStorage.getItem('selectedAssets')) || [];
            document.querySelectorAll('#asetTableBody tr').forEach(function(row) {
                const nomorAset = row.querySelector('.pilihAset')?.dataset.nomor;
                if (nomorAset) {
                    row.style.display = selectedAssets.some(function(a) { return a.nomor === nomorAset; }) ? 'none' : '';
                }
            });
        },
        error: function(xhr, status, error) {
            console.error('Error load all assets:', error);
            alert('Gagal memuat data aset: ' + error);
        }
    });
}


        // Saat modal akan dibuka: reset search dan muat ulang data aset
        document.getElementById('assetModal').addEventListener('show.bs.modal', function() {
    ...
    <script>
        // Populate Job Role and Alamat based on the selected user (Pihak Pertama)
