        ...
        <div class="container">
            <div class="row row-cols-1 gy-3 p-3 p-lg-4">
                    ...
                    <div class="d-flex flex-column gap-2">
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                            ...
                            <ol class="breadcrumb mb-1">
                                @if (request()->routeIs('bast-aset.index'))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Berita
                                            Acara</a></li>
                                @elseif(request()->routeIs('daftar-tanda-tangan.bast'))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Tanda
                                            Tangan</a></li>
                                @endif

                                <li class="breadcrumb-item active" aria-current="page">BA Serah Terima Aset</li>
                ...
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-body">
                        <h4 class="m-0">Berita Acara Serah Terima Aset</h4>
                        @if (request()->routeIs('daftar-tanda-tangan.bast'))
                            <span class="text-muted">Di bawah ini adalah daftar dokumen BA Serah Terima aset yang perlu Anda
                                tanda tangani.</span>
                        @endif
                        ...
                        <div class="table-responsive mt-4">
                            <table class="table table-bordered align-middle">
                                ...
                                <tbody>
                                    @forelse ($bastAsets as $bastAset)
                                        <tr>
                                            <td>{{ ($bastAsets->currentPage() - 1) * $bastAsets->perPage() + $loop->iteration }}
                                            </td>
                                            <td>{{ $bastAset->nomor_surat ?? '-' }}</td>
                                            <td>{{ \Carbon\Carbon::parse($bastAset->tanggal)->format('d/m/Y') }}</td>
                                            <td>{{ $bastAset->pihakPertama->name_karyawan ?? '-' }}</td>
                                            <td>{{ $bastAset->pihakKedua->name_karyawan ?? '-' }}</td>
                                            @if (request()->routeIs('daftar-tanda-tangan.bast'))
                                                <td style="text-align: center;">
                                                    @if (is_null($bastAset->tanda_tangan_pihak_kedua))
                                                        <span
                                            ...
                                            <td style="width: 124px;">
                                                <div class="d-flex align-items-center justify-content-center gap-1">
                                                    @if (request()->routeIs('bast-aset.index'))
                                                        <a href="{{ route('bast-aset.show', $bastAset->id) }}"
                                                            class="btn btn-icon btn-sm btn-outline-secondary"
                                                            aria-label="Lihat Surat" title="Pertinjau Dokumen">
                                                            <i class="sym sym-eye-solid"></i>
                                                        </a>
                                                    @elseif(request()->routeIs('daftar-tanda-tangan.bast'))
                                                        <a href="{{ route('daftar-tanda-tangan.detailBast', $bastAset->id) }}"
                                                            class="btn btn-icon btn-sm btn-outline-secondary"
                                                            aria-label="Lihat Surat" title="Pertinjau Dokumen">
                                                            <i class="sym sym-eye-solid"></i>
                                                        </a>
                                                    ...
                                                    @can('akses-admin-superadmin')
                                                        <a href="{{ route('bast-aset.edit', $bastAset->id) }}">
                                                            <button type="button"
                                                                class="btn btn-icon btn-sm btn-outline-secondary"
                                                        ...
                                                        <form id="delete-form-{{ $bastAset->id }}"
                                                            action="{{ route('bast-aset.destroy', $bastAset->id) }}"
                        ...
                        <div
                            ...
                            <p class="text-dark m-0">
                                Menampilkan
                                <span class="fw-bold">{{ $bastAsets->firstItem() }}</span> -
                                <span class="fw-bold">{{ $bastAsets->lastItem() }}</span>
                                dari Total <span class="fw-bold">{{ $bastAsets->total() }}</span> data
                            </p>
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-end mb-0">
                                    ...
                                    @php
                                        $currentPage = $bastAsets->currentPage();
                                        $lastPage = $bastAsets->lastPage();
                                    @endphp

                                    <!-- Tombol Sebelumnya -->
                                    <li class="page-item {{ $bastAsets->onFirstPage() ? 'disabled' : '' }}">
                                        <a class="page-link"
                                            href="{{ $bastAsets->appends(request()->query())->previousPageUrl() }}">
                                    ...
                                    @for ($page = 1; $page <= 3 && $page <= $lastPage; $page++)
                                        <li class="page-item {{ $currentPage == $page ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $bastAsets->appends(request()->query())->url($page) }}">{{ $page }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if ($lastPage > 3)
                                        <li class="page-item {{ $currentPage == $lastPage ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $bastAsets->appends(request()->query())->url($lastPage) }}">{{ $lastPage }}</a>
                                        </li>
                                    @endif

                                    <!-- Tombol Selanjutnya -->
                                    <li class="page-item {{ $bastAsets->onLastPage() ? 'disabled' : '' }}">
                                        <a class="page-link"
                                            href="{{ $bastAsets->appends(request()->query())->nextPageUrl() }}">
