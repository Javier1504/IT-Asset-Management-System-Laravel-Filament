...
@section('title', 'Edit Pemusnahan Aset')
...
@section('content')
    <header class="qn-header z-1 sticky-top p-md-3 py-3 px-xl-5 bg-white">
        <div class="container-fluid d-grid d-flex justify-content-between align-items-center position-relative">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="m-0 fs-6 fw-medium">Edit Pemusnahan Aset</span>
            </div>
        ...
        <div class="container-fluid p-0">
            <div class="w-100 p-2 bg-white">
                <div class="w-100 bg-body-tertiary rounded-4 p-2 py-md-3 py-xl-4 pb-5">
                    <form method="POST" action="{{ route('pemusnahan-aset.update', $pemusnahan->id) }}" id="pemusnahanForm"
                                    ...
                                    <div class="row row-cols-1 row-cols-md-2 g-3">
                                        ...
                                        <div class="col-md-12 mt-0">
                                            <div class="d-flex flex-column gap-1 mb-2">
                                                <h1 class="fs-5 fw-medium mb-0">Data Pemusnahan Aset</h1>
                                        ...
                                        <div class="col-md-12 mt-3">
                                            ...
                                            <textarea class="form-control" id="keterangan" name="keterangan" rows="3"
                                                placeholder="Masukkan keterangan tambahan jika ada..." required>{{ $pemusnahan->keterangan }}</textarea>
                                        </div>
                                        {{-- Lokasi Pemusnahan --}}
                                        <div class="col-md-6 mt-3">
                                            ...
                                            <textarea class="form-control" id="lokasi" name="lokasi" rows="2" placeholder="Masukkan lokasi ..."
                                                required>{{ $pemusnahan->lokasi_pemusnahan }}</textarea>
                                        </div>
                                        {{-- Metode Pemusnahan --}}
                                        <div class="col-md-6 mt-3">
                                            ...
                                            <textarea class="form-control" id="metode" name="metode" rows="2"
                                                placeholder="Masukkan metode pemusnahan aset yang digunakan..." required>{{ $pemusnahan->metode_pemusnahan }}</textarea>
                                        </div>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="file" class="form-control" id="lampiran_sebelum"
                                                        name="lampiran_sebelum[]" multiple accept=".jpg,.jpeg,.png">
                                                    <div class="form-text">Format yang diizinkan: JPG, JPEG, PNG
                                                        Maksimal 5MB per file.</div>
                                                </div>

                                                <!-- Gambar yang sudah ada -->
                                                @if ($pemusnahan->pemusnahanBuktis->where('type', 'before')->count() > 0)
                                                    <div class="existing-images mt-2 d-flex flex-wrap gap-2">
                                                        @foreach ($pemusnahan->pemusnahanBuktis->where('type', 'before') as $bukti)
                                                            <div class="position-relative"
                                                                id="existing-before-{{ $bukti->id }}">
                                                                ...
                                                                <button type="button"
                                                                    ...
                                                                    onclick="removeExistingImage({{ $bukti->id }}, 'before')">
                                                                    <svg width="14" height="14"
                                                                        viewBox="0 0 16 16">
                                                                        <path d="M2 2l12 12M14 2L2 14" stroke="#fff"
                                                                            stroke-width="2" />
                                                                    </svg> </button>
                                                                <input type="hidden" name="delete_bukti[]"
                                                                    id="delete-before-{{ $bukti->id }}"
                                                                    value="">
                                                            </div>
                                                        @endforeach
                                                    </div>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="file" class="form-control" id="lampiran_sesudah"
                                                        name="lampiran_sesudah[]" multiple accept=".jpg,.jpeg,.png">
                                                    <div class="form-text">Format yang diizinkan: JPG, JPEG, PNG
                                                        Maksimal 5MB per file.</div>
                                                </div>

                                                <!-- Gambar yang sudah ada -->
                                                @if ($pemusnahan->pemusnahanBuktis->where('type', 'after')->count() > 0)
                                                    <div class="existing-images mt-2 d-flex flex-wrap gap-2">
                                                        @foreach ($pemusnahan->pemusnahanBuktis->where('type', 'after') as $bukti)
                                                            <div class="position-relative"
                                                                id="existing-after-{{ $bukti->id }}">
                                                                <img src="{{ asset('storage/' . $bukti->file_path) }}"
                                                                    class="img-thumbnail"
                                                                    style="width: 100px; height: 100px; object-fit: cover;">
                                                                <button type="button"
                                                                    class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1"
                                                                    style="padding: 2px 6px; font-size: 12px;"
                                                                    onclick="removeExistingImage({{ $bukti->id }}, 'after')">
                                                                    <svg width="14" height="14"
                                                                        viewBox="0 0 16 16">
                                                                        <path d="M2 2l12 12M14 2L2 14" stroke="#fff"
                                                                            stroke-width="2" />
                                                                ...
                                                                <input type="hidden" name="delete_bukti[]"
                                                                    id="delete-after-{{ $bukti->id }}" value="">
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                @endif
    ...
    <script>
        ...
        // Preview gambar baru sebelum
        document.getElementById('lampiran_sebelum').addEventListener('change', function(e) {
            const preview = document.getElementById('preview-sebelum');
            filesSebelum = Array.from(e.target.files);
            preview.innerHTML = '';

            filesSebelum.forEach((file, index) => {
                if (file.type.startsWith('image/')) {
                    ...
                    <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1" 
                        style="padding:2px 6px;font-size:12px;" onclick="removeImageSebelum(${index})">
        ...
        // Preview gambar baru sesudah
        document.getElementById('lampiran_sesudah').addEventListener('change', function(e) {
            const preview = document.getElementById('preview-sesudah');
            filesSesudah = Array.from(e.target.files);
            preview.innerHTML = '';

            filesSesudah.forEach((file, index) => {
                if (file.type.startsWith('image/')) {
                    const reader = new FileReader();
                    reader.onload = function(e) {
                        const div = document.createElement('div');
                    ...
                    <img src="${e.target.result}" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
                    <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1" 
                        style="padding: 2px 6px; font-size: 12px;" onclick="removeImageSesudah(${index})">
                        <svg width="14" height="14" viewBox="0 0 16 16">
                          <path d="M2 2l12 12M14 2L2 14" stroke="#fff" stroke-width="2"/>
                        </svg>                      
                    </button>
        ...
        // Hapus gambar baru
        function removeImageSebelum(index) {
            filesSebelum.splice(index, 1);
            const dataTransfer = new DataTransfer();
            filesSebelum.forEach(file => dataTransfer.items.add(file));
            document.getElementById('lampiran_sebelum').files = dataTransfer.files;
            document.getElementById('lampiran_sebelum').dispatchEvent(new Event('change'));
        }

        function removeImageSesudah(index) {
            filesSesudah.splice(index, 1);
            const dataTransfer = new DataTransfer();
            filesSesudah.forEach(file => dataTransfer.items.add(file));
            document.getElementById('lampiran_sesudah').files = dataTransfer.files;
            document.getElementById('lampiran_sesudah').dispatchEvent(new Event('change'));
        }

        // Hapus gambar existing
        function removeExistingImage(id, type) {
            document.getElementById(`existing-${type}-${id}`).style.display = 'none';
    ...
    <script>
        ...
        // Load existing data saat halaman dibuka
        document.addEventListener("DOMContentLoaded", function() {
            // Load data pelaksana dan pihak terlibat yang sudah ada
            loadExistingData();

            // Load aset yang sudah ada ke localStorage
            loadExistingAssets();
        ...
        function loadExistingData() {
            const pemusnahanData = @json($pemusnahan);

            // Load Tim Pelaksana
            const timPelaksana = pemusnahanData.pemusnahan_users.filter(u => u.kategori === 'tim_pelaksana');
            timPelaksana.forEach((item, index) => {
                if (index === 0) {
                    // Set pelaksana pertama yang sudah ada
                    const select = document.querySelector('[name="tim_pelaksana[0][user_id]"]');
                    const jobRole = document.getElementById('jobRolePel0');
                    const peran = document.querySelector('[name="tim_pelaksana[0][peran]"]');

                    if (select) select.value = item.user_id;
                    if (jobRole) jobRole.value = item.user?.job_role || '';
                    if (peran) peran.value = item.peran;
                } else {
                    // Tambah pelaksana berikutnya
                    addExistingPelaksana(item, index);
                }
            });

            // Update pelaksanaIndex
            pelaksanaIndex = timPelaksana.length;

            // Load Pihak Terlibat
            const pihakTerlibat = pemusnahanData.pemusnahan_users.filter(u => u.kategori === 'pihak_terlibat');
            pihakTerlibat.forEach((item, index) => {
                addExistingPihakTerlibat(item, index);
            });

            // Update pihakIndex
            pihakIndex = pihakTerlibat.length;
        }

        // Tambah pelaksana yang sudah ada
        function addExistingPelaksana(data, index) {
            ...
            const userOptions = @json($users);
            let optionsHtml = '<option value="">-- Pilih Pelaksana ' + (index + 1) + ' --</option>';
            userOptions.forEach(user => {
                const selected = data.user_id == user.id ? 'selected' : '';
                optionsHtml +=
                    `<option value="${user.id}" data-role="${user.job_role || ''}" ${selected}>${user.name_karyawan}</option>`;
            });

            newItem.innerHTML = `
            <div class="col-md-4">
                <label class="form-label">Pelaksana ${index + 1}</label>
                <select class="form-select petugas-select" required name="tim_pelaksana[${index}][user_id]" 
                        data-target-role="jobRolePel${index}">
                    ${optionsHtml}
                </select>
            </div>
            <div class="col-md-4">
                <label class="form-label">Job Role <span class="text-danger">*</span></label>
                <input type="text" id="jobRolePel${index}" name="tim_pelaksana[${index}][job_role]" 
                       class="form-control" placeholder="Job Role" value="${data.user?.job_role || ''}" readonly>
            </div>
            <div class="col-md-3">
                <label class="form-label">Peran Pelaksana ${index + 1} <span class="text-danger">*</span></label>
                <input type="text" name="tim_pelaksana[${index}][peran]" class="form-control" 
                       placeholder="Peran Pelaksana ${index + 1}" value="${data.peran}" required>
            </div>
        ...
        // Tambah pihak terlibat yang sudah ada
        function addExistingPihakTerlibat(data, index) {
            ...
            const userOptions = @json($users);
            let optionsHtml = '<option value="">-- Pilih Pihak Terlibat ' + (index + 1) + ' --</option>';
            userOptions.forEach(user => {
                const selected = data.user_id == user.id ? 'selected' : '';
                optionsHtml +=
                    `<option value="${user.id}" data-role="${user.job_role || ''}" ${selected}>${user.name_karyawan}</option>`;
            ...
            newItem.innerHTML = `
            <div class="col-md-4">
                <label class="form-label">Pihak Terlibat ${index + 1}</label>
                <select class="form-select petugas-select" required name="pihak_terlibat[${index}][user_id]" 
                        data-target-role="jobRolePihak${index}">
                    ${optionsHtml}
                </select>
            </div>
            <div class="col-md-4">
                <label class="form-label">Job Role <span class="text-danger">*</span></label>
                <input type="text" id="jobRolePihak${index}" name="pihak_terlibat[${index}][job_role]" 
                       class="form-control" placeholder="Job Role" value="${data.user?.job_role || ''}" readonly>
            </div>
            <div class="col-md-3">
                <label class="form-label">Peran Pihak ${index + 1} <span class="text-danger">*</span></label>
                <input type="text" name="pihak_terlibat[${index}][peran]" class="form-control" 
                       placeholder="Peran Pihak ${index + 1}" value="${data.peran}" required>
            </div>
            <div class="col-md-1 d-flex align-items-end">
                <button type="button" class="btn btn-danger w-100 removePetugas" title="Hapus">
                    <i class="sym sym-trash"></i>
        ...
        // Load aset yang sudah ada
        function loadExistingAssets() {
            const pemusnahanData = @json($pemusnahan);
            const existingAssets = [];

            pemusnahanData.pemusnahan_asets.forEach(item => {
                const asset = {
                    qty: item.qty,
                    asetId: item.aset_id,
                    sparepartId: item.sparepart_id,
                    jenis: item.manual_jenis || item.aset?.jenis_aset?.name_jenis || item.sparepart
                        ?.jenis_sparepart?.jenis_sparepart || '-',
                    merk: item.manual_merk || item.aset?.merk_aset || item.sparepart?.nama_sparepart || '-',
                    nomor: item.manual_nomor || item.aset?.nomor_aset || item.sparepart?.nomor_sparepart || '-'
                };
                existingAssets.push(asset);
            });

            localStorage.setItem('selectedAssets', JSON.stringify(existingAssets));
        }
        ...
        window.addEventListener("beforeunload", function(e) {
            if (!document.getElementById('pemusnahanForm').submitted) {
                localStorage.removeItem('selectedAssets');
            }
        });

        document.getElementById('pemusnahanForm').addEventListener('submit', function() {
            this.submitted = true;
        });
