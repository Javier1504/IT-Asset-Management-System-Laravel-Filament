...
@section('content')
    ...
    <header class="qn-header z-1 sticky-top p-md-3 py-3 px-xl-5 bg-white">
        <div class="container-fluid d-grid d-flex justify-content-between align-items-center position-relative">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="m-0 fs-6 fw-medium">Edit Surat BAST Pengembalian Aset</span>
            </div>
        ...
        <div class="container-fluid p-0">
            <div class="w-100 p-2 bg-white">
                <div class="w-100 bg-body-tertiary rounded-4 p-2 py-md-3 py-xl-4 pb-5">
                    <form method="POST" action="{{ route('bast-pengembalian.update', $bastAset->id) }}" id="advancedForm"
                                ...
                                <div class="card-body p-3 px-md-4 p-xl-4 px-xl-5">
                                    <div class="row row-cols-1 row-cols-md-2 g-3">
                                            ...
                                            <select class="form-select" id="inputUser1" name="petugas_id">
                                                ...
                                                @foreach ($users->where('role', 'admin') as $user)
                                                    <option value="{{ $user->id }}"
                                                        ...
                                                        {{ old('petugas_id', $bastAset->petugas_id ?? '') == $user->id ? 'selected' : '' }}>
                                                        {{ $user->name_karyawan }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-select" id="inputUser2" name="pengembali_id">
                                                ...
                                                @foreach ($pengembali as $user)
                                                    <option value="{{ $user->id }}"
                                                        data-job-role="{{ $user->job_role }}"
                                                        data-alamat="{{ $user->alamat }}"
                                                        {{ old('pengembali_id', $bastAset->pengembali_id ?? '') == $user->id ? 'selected' : '' }}>
                                                        {{ $user->name_karyawan }}
                                                    </option>
                                                @endforeach
                                            </select>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="tab-content mt-3" id="assetTabContent">
                                                            ...
                                                            <div class="tab-pane fade show active" id="endUserAset"
                                                                role="tabpanel" aria-labelledby="endUserAset-tab">
                                                                <div
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <div class="row">
                                                @php
                                                    // Ambil kondisi yang sudah ada, kalau tidak ada cek dari old()
                                                    $selectedKondisi = old(
                                                        'kondisi_aset',
                                                        $bastAset->bastPengembalianDetails->first()->kondisi_aset ?? '',
                                                    );
                                                @endphp
                                                    ...
                                                    @foreach (['baik', 'rusak_berat'] as $kondisi)
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="radio"
                                                                ...
                                                                value="{{ $kondisi }}"
                                                                {{ $selectedKondisi == $kondisi ? 'checked' : '' }}
                                                                required>
                                                ...
                                                <div class="col-md-12 mt-2 d-flex align-items-center">
                                                    <div class="form-check me-2">
                                                        <input class="form-check-input" type="radio"
                                                            ...
                                                            {{ $selectedKondisi == 'lainnya' ? 'checked' : '' }}>
                                                    ...
                                                    <input type="text" class="form-control w-25"
                                                        ...
                                                        value="{{ old('kondisi_lainnya', $bastAset->bastPengembalianDetails->first()->kondisi_lainnya ?? '') }}"
                                                        {{ $selectedKondisi == 'lainnya' ? '' : 'disabled' }}>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        let selectedPihakKeduaId = null;
        let prevPihakKeduaId = null;
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            $('#btnSearch').on('click', function(e) {
                ...
                selectedPihakKeduaId = $('#inputUser2').val();
                if (!selectedPihakKeduaId) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Pilih Pihak Kedua',
                        text: 'Pilih penerima terlebih dahulu sebelum mencari aset.'
                    });
                    return;
                }
            ...
            function renderSelectedAssets() {
                ...
                document.querySelectorAll('.removeDbAsset').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        Swal.fire({
                            title: 'Hapus Aset?',
                            text: 'Aset ini akan dihapus dari daftar.',
                            icon: 'warning',
                            ...
                            if (result.isConfirmed) {
                                ...
                                Swal.fire({
                                    title: 'Terhapus!',
                                    text: 'Aset berhasil dihapus dari daftar.',
                                    icon: 'success',
                                    timer: 1500,
                ...
                document.querySelectorAll('.removeAsset').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                let selectedAssets = JSON.parse(localStorage.getItem(
                                    'selectedAssets')) || [];
                                selectedAssets.splice(index, 1);
                                localStorage.setItem('selectedAssets', JSON.stringify(
                                    selectedAssets));
                                renderSelectedAssets();
                                Swal.fire({
                                    title: 'Terhapus!',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data && response.data.length > 0) {
                            ...
                            container.html(
                                '<tr><td colspan="9" class="text-center">Tidak ada data aset yang tersedia untuk user ini</td></tr>'
                            );
                        }
                    },
                    error: function(xhr, status, error) {
                        ...
                        $('#resultAsetContainer').html(
                            '<tr><td colspan="9" class="text-center text-danger">Gagal memuat data aset: ' +
                            error + '</td></tr>'
                        );
                    }
                });
            ...
            // Guard modal opening - require Pihak Kedua selection first
            document.getElementById('assetModal').addEventListener('show.bs.modal', function(e) {
                const val = document.getElementById('inputUser2').value;
                if (!val) {
                    e.preventDefault();
                    Swal.fire({
                        icon: 'warning',
                        title: 'Pilih Pihak Kedua',
                        text: 'Pilih penerima terlebih dahulu sebelum menambah aset.'
                    });
                    return;
                }
                selectedPihakKeduaId = val;
                // Refresh data modal sesuai user
                loadAllAsets();
            });

            // Handle perubahan Pihak Kedua: kosongkan aset terpilih bila sudah ada dan load data baru
            document.getElementById('inputUser2').addEventListener('change', function() {
                const selectedAssets = JSON.parse(localStorage.getItem('selectedAssets')) || [];
                const newVal = this.value;

                if (selectedAssets.length > 0 || databaseAssets.length > 0) {
                    Swal.fire({
                        ...
                        if (res.isConfirmed) {
                            ...
                            const prevOpt = Array.from(this.options).find(o => o.value == (
                                prevPihakKeduaId || ''));
    ...
    <script>
        document.querySelectorAll('input[name="kondisi_aset"]').forEach(radio => {
            radio.addEventListener('change', function() {
                const lainnyaField = document.getElementById('kondisi_lainnya_field');

                if (this.value === 'lainnya') {
                    lainnyaField.removeAttribute('disabled');
                } else {
                    lainnyaField.setAttribute('disabled', 'disabled');
                    lainnyaField.value = '';
                }
            });
        });
