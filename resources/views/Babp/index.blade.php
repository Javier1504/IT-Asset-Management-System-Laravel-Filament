...
@section('title', 'Page All Surat BABP')
            ...
            <div class="row row-cols-1 gy-3 p-3 p-lg-4">
                    ...
                    <div class="d-flex flex-column gap-2">
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                            <ol class="breadcrumb mb-1">
                                @if (request()->routeIs('babp.index'))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Berita
                                            Acara</a></li>
                                @elseif(request()->routeIs('daftar-tanda-tangan.babp'))
                                    <li class="breadcrumb-item"><a href="#"><i class="sym sym-home-line"></i> Tanda
                                            Tangan</a></li>
                                @endif

                                <li class="breadcrumb-item active" aria-current="page">BA Bukti Pembelian</li>
                ...
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-body">
                        ...
                        <h4 class="m-0">Berita Acara Bukti Pembelian Aset</h4>
                        @if (request()->routeIs('daftar-tanda-tangan.babp'))
                            <!-- Sesuaikan dengan nama route -->
                            <span class="text-muted">Di bawah ini adalah daftar dokumen BA Bukti Pembelian aset yang perlu
                                Anda tanda tangani.</span>
                        @endif
                            ...
                            <table class="table table-bordered align-middle">
                                <thead class="align-middle">
                                    <tr class="table-light">
                                        ...
                                        <th style="min-width: 180px; width: 10%;">
                                            <button class="btn p-0 border-0 w-100 h-100 text-start" aria-label="Validator">
                                                Validator
                                                <i class="float-end sym sym-switch-vertical"></i>
                                            </button>
                                        </th>
                                        <th style="min-width: 140px; width: 10%;">
                                            <button class="btn p-0 border-0 w-100 h-100 text-start" aria-label="Status">
                                                Status Progress
                                                <i class="float-end sym sym-switch-vertical"></i>
                                            </button>
                                        </th>
                                ...
                                <tbody>
                                    @forelse ($babps as $babp)
                                        <tr>
                                            <td>{{ ($babps->currentPage() - 1) * $babps->perPage() + $loop->iteration }}
                                            </td>
                                            <td>{{ $babp->nomor_surat ?? '-' }}</td>
                                            <td>{{ \Carbon\Carbon::parse($babp->tanggal_surat)->format('d/m/Y') }}</td>
                                            <td>{{ $babp->petugas->name_karyawan ?? '-' }}</td>
                                            <td>{{ $babp->verifier->name_karyawan ?? '-' }}</td>
                                            <td>{{ $babp->penerima->name_karyawan ?? '-' }}</td>
                                            ...
                                            @if (request()->routeIs('daftar-tanda-tangan.babp'))
                                                <td style="text-align: center;">
                                                    @if (is_null($babp->tanda_tangan_penerima))
                                                        <span
                                                ...
                                                <div class="d-flex align-items-center justify-content-center gap-1">
                                                    ...
                                                    @can('akses-admin-superadmin')
                                                        <button type="button" class="btn btn-icon btn-sm btn-outline-secondary"
                                                            ...
                                                            onclick="confirmDeletion({{ $babp->id }})">
                                                            <i class="sym sym-trash-solid"></i>
                                                        </button>
                                                        <form id="delete-form-{{ $babp->id }}"
                                                            action="{{ route('babp.destroy', $babp->id) }}" method="POST"
                                                            style="display: none;">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                        <a href="{{ route('babp.edit', $babp->id) }}">
                                                            <button type="button"
                                                                class="btn btn-icon btn-sm btn-outline-secondary"
                        ...
                        <div
                            ...
                            <p class="text-dark m-0">
                                Menampilkan
                                <span class="fw-bold">{{ $babps->firstItem() }}</span> -
                                <span class="fw-bold">{{ $babps->lastItem() }}</span>
                                dari Total <span class="fw-bold">{{ $babps->total() }}</span> data
                            </p>
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-end mb-0">
                                    ...
                                    @php
                                        $currentPage = $babps->currentPage();
                                        $lastPage = $babps->lastPage();
                                    @endphp

                                    <!-- Tombol Sebelumnya -->
                                    <li class="page-item {{ $babps->onFirstPage() ? 'disabled' : '' }}">
                                        <a class="page-link"
                                            href="{{ $babps->previousPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">
                                    ...
                                    @for ($page = 1; $page <= 3 && $page <= $lastPage; $page++)
                                        <li class="page-item {{ $currentPage == $page ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $babps->url($page) }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">{{ $page }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if ($lastPage > 3)
                                        <li class="page-item {{ $currentPage == $lastPage ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $babps->url($lastPage) }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">{{ $lastPage }}</a>
                                        </li>
                                    @endif

                                    <!-- Tombol Selanjutnya -->
                                    <li class="page-item {{ $babps->onLastPage() ? 'disabled' : '' }}">
                                        <a class="page-link"
                                            href="{{ $babps->nextPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">
