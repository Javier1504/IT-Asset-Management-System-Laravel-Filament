                    ...
                    <div class="card-body">
                        ...
                        <div class="row d-flex align-items-center justify-content-between gap-2">
                            ...
                            <div class="col d-flex justify-content-end gap-2 flex-wrap">
                                <a href="{{ route('jenis-aset.download-template') }}"
                                ...
                                <a href="{{ route('jenis-aset.create') }}"
                        ...
                        <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route('jenis-aset.import-template') }}" method="POST"
                        ...
                        <div class="table-responsive mt-4">
                            <table class="table table-bordered align-middle">
                                ...
                                <tbody>
                                    @foreach ($paginator as $jenis)
                                        <tr>
                                            <td>{{ ($paginator->currentPage() - 1) * $paginator->perPage() + $loop->iteration }}
                                            </td>
                                            <td>{{ $jenis->name_jenis }}</td>
                                            <td>
                                                @if ($jenis->category == 'end_user_aset')
                                                    <span>End User Aset</span>
                                                @elseif ($jenis->category == 'physical_host_aset')
                                                    <span>Physical Host Aset</span>
                                                @elseif ($jenis->category == 'office_aset')
                                                    <span>Office Aset</span>
                                                @elseif ($jenis->category == 'security_peripheral')
                                                    <span>Security Peripheral</span>
                                                @else
                                                    <span>{{ $jenis->category }}</span>
                                                @endif
                                            </td>
                                            <td>{{ $jenis->jumlah }}</td> {{-- pakai property jumlah hasil hitung di controller --}}
                                            <td style="width: 124px;">
                                                <div class="d-flex align-items-center justify-content-center gap-1">
                                                    <button type="button"
                                                        class="btn btn-icon btn-sm btn-outline-secondary"
                                                        aria-label="Hapus" title="Hapus"
                                                        onclick="confirmDeletion({{ $jenis->id }})">
                                                        <i class="sym sym-trash-solid"></i>
                                                    </button>

                                                    <form id="delete-form-{{ $jenis->id }}"
                                                        action="{{ route('jenis-aset.destroy', $jenis->id) }}"
                                                        method="POST" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                    <a href="{{ route('jenis-aset.edit', $jenis->id) }}">
                                                        <button type="button"
