    ...
    <table class="table table-hover table-bordered align-middle">
        <thead class="table-light">
            <tr>
                ...
                <th>Alasan</th>
                <th>Pemegang</th>
                <th>No Aset Tujuan</th>
                <th style="width: 120px;" class="text-center">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($checkouts as $entry)
                <tr>
                    <td>{{ ($checkouts->currentPage() - 1) * $checkouts->perPage() + $loop->iteration }}</td>
                    <td>{{ \Carbon\Carbon::parse($entry->tanggal)->format('d/m/Y') }}</td>
                    <td>{{ $entry->qty }}</td>
                    <td>{{ $entry->alasan ?? '-' }}</td>
                    <td>{{ $entry->user->name_karyawan ?? '-' }}</td>
                    <td>{{ $entry->aset->nomor_aset ?? '-' }}</td>
                    <td>
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center text-muted">Tidak ada data check-out.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
@if ($checkouts->hasPages())
    <div class="d-flex flex-column-reverse flex-md-row align-items-center justify-content-between gap-3 pt-4">
        <p class="text-dark m-0">
            Menampilkan
            <span class="fw-bold">{{ $checkouts->firstItem() ?? 0 }}</span> -
            <span class="fw-bold">{{ $checkouts->lastItem() ?? 0 }}</span>
            dari Total <span class="fw-bold">{{ $checkouts->total() }}</span> data
        </p>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-end mb-0">
                @php
                    ...
                    $lastPage = $checkouts->lastPage();
                @endphp
                <li class="page-item {{ $checkouts->onFirstPage() ? 'disabled' : '' }}">
                    <a class="page-link"
                        href="{{ $checkouts->previousPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">
                ...
                @for ($page = 1; $page <= 3 && $page <= $lastPage; $page++)
                    <li class="page-item {{ $currentPage == $page ? 'active' : '' }}">
                        <a class="page-link"
                            href="{{ $checkouts->url($page) }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">{{ $page }}</a>
                    </li>
                @endfor
                ...
                @if ($lastPage > 3)
                    <li class="page-item {{ $currentPage == $lastPage ? 'active' : '' }}">
                        <a class="page-link"
                            href="{{ $checkouts->url($lastPage) }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">{{ $lastPage }}</a>
                    </li>
                @endif

                <li class="page-item {{ $checkouts->onLastPage() ? 'disabled' : '' }}">
                    <a class="page-link"
                        href="{{ $checkouts->nextPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}">
